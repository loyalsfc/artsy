...
function DropItem({img, title, creator, status, time}) {
    ...
    return (
        <div className='flex gap-10 mb-20 '>
            <div className='w-1/2 shrink-0 relative px-8 pb-4 rounded-[10px] overflow-hidden flex justify-end flex-col'>
                <img src={img} alt="" className='absolute w-full left-0 bottom-0'/>
                <div className="w-full text-white backdrop-blur-[1.5px] flex items-end justify-between rounded-[0.5rem] border-[0.5px] border-white lightbg pt-4 pr-2 pl-8 pb-2">
                    ...
                    <button className={`rounded-[15px] bg-${status == "ended" ? '[#B8BABC]' : 'blue'} font-medium leading-[95%] text-sm py-1.5 px-6`}>{status == "ended" ? 'View' : 'Join'}</button>
                </div>
            </div>
            <article className='flex flex-col items-start gap-8'>
                ...
                <p className='text-black font-medium'>Creator: <span className='text-[#006CA2]'>{toTitleCase(creator)}</span> </p>
                <span className='text-[#006CA2] underline text-sm'>{linkText}</span>
            </article>
        </div>
    )
}
